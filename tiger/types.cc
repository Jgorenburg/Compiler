...
bool Ty_listTypeEqual(Ty_tyList first, Ty_tyList second)
{
	if (first == 0 || second == 0) { return first == 0 && second == 0; }  								// if lists are empty
	else if (first->tail == 0 || second->tail == 0) { return (first->tail == 0 && second->tail == 0) && // if the lists have no tail, check if heads are the same type
															 (first->head == second->head) ;
													}
	else { return (first->head == second->head) && Ty_listTypeEqual(first->tail, second->tail); }       // if it isn't empty, compare heads and tails
}
