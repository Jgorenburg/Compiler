...
public:
	...
	virtual string HERA_data();  // defaults to a warning

	virtual std::list<AST_node_*> getChildren(); // list of the children of the node
	...
	// register and DLABEL information
	...
	// Gives node a unique DLABEL in the same way as result_reg gives a unique register
	int cur_DLABEL() {
		if (this->stored_DLABEL < 0) this->stored_DLABEL = this->init_DLABEL();
	...
	string cur_DLABEL_s() {
		return "D" + std::to_string(this->cur_DLABEL()); //adding D because it needs to be a string
	}

	virtual int init_result_reg();
	virtual int init_DLABEL();
	virtual string gen_LABEL();

	virtual Ty_ty get_type() { return 0; }
	virtual void type_check() { };

	//ideally put in a .cc at some point
	//ST_bi init_ST();
	virtual void set_ST();
	virtual void set_ST_to(ST_bi new_ST);
	ST_bi get_ST();
	void merge_ST(ST_bi to_add);
	virtual void send_or_recieve_ST(ST_bi new_ST) {
		EM_error("Send_ST should not trigger for an AST_node", true);
	}

	ST_bi bi_ST = ST_bi();

	// for all node but for and while, get_exit checks its parent
	// if a root node tries this, it will crash
	virtual string get_exit() { return this->parent()->get_exit(); }

	virtual int get_SP() {
		if (this->SP == -1) { this->SP = this->parent()->get_SP(); }
		return SP;
	}
	virtual string get_SP_s() {
		return str(this->get_SP());
	}
	virtual void set_SP(int to_set) {
		this->SP = to_set;
	}
	virtual void inc_SP(int amount) {
		this->SP = this->get_SP() + amount;
	}
	virtual int get_SP_and_inc() {
		int ret_SP = this->get_SP();
		this->inc_SP(1);
		return ret_SP;
	}
	virtual void send_or_recieve_SP(int new_SP) {
		EM_error("Send_SP should not trigger for an AST_node", true);
	}
...
private:
	...
	int stored_DLABEL = -1; 		// Initialize to -1 to be sure it gets replaced by "if" in cur_DLABEL() above
	int SP = -1;							// Initialize to -999 to be sure it gets replaced by "if" in Get_SP() above
...
public:
	...
	virtual std::list<AST_node_*> getChildren();
	...
	virtual void set_ST();
	virtual ST_bi init_ST();

	virtual int get_SP() { return 0; }	// always starts at 0;
...
//	virtual string HERA_data();
	virtual std::list<AST_node_*> getChildren(); // should always return 0
};
...
//	void set_parent_pointers_for_me_and_my_decendents(AST_node_ *my_parent);
	...
	virtual Ty_ty get_type();
	virtual void type_check(bool comp = false);
...
public:
	...
	virtual Ty_ty get_type() { return Ty_Void(); }
	virtual void type_check();
...
public:
	...
	virtual void send_or_recieve_ST(ST_bi new_ST); 													// recieves ST from defs
	virtual void send_or_recieve_SP(int new_SP) { this->set_SP(new_SP); }	 	// recieves SP from defs
...
//	void set_parent_pointers_for_me_and_my_decendents(AST_node_ *my_parent);
	...
	virtual string get_exit() { return exit; }
...
public:
	A_var_(A_pos p);
	virtual Ty_ty get_type() {
		EM_error("A_var get_type() should not be called", true);
		return Ty_Error();
	}
	virtual bool valid_var() {
		EM_error("A_var valid_var() should not be called", true);
		return false;
	}
	virtual Symbol get_var() {
		EM_error("A_var get_var() should not be called", true);
		return to_Symbol("Should never return this");
	}
...
public:
	...
	virtual Symbol get_var() { return _sym; }
...
public:
	...
	virtual string print_rep(int indent, bool with_attributes);
	virtual string HERA_code(string soFar = "", int count = 1, bool store = false);
	virtual string HERA_code(bool store) { return this->HERA_code("", 1, store);	}
//	virtual string HERA_data(string soFar = "");
	virtual std::list<AST_node_*> getChildren();
	int length();
	A_exp _head;
	A_expList _tail;
	...
	A_exp get_last();
	Ty_tyList init_listType();
	Ty_tyList get_listType();

	//recursively designed
	Ty_ty get_type_last();

	Ty_tyList list_type = 0;
...
// The components of a A_recordExp, e.g. point{X = 4, Y = 12}
...
public:
	...
	virtual string print_rep(int indent, bool with_attributes);
	virtual string HERA_code(string soFar = "");
	virtual int length();
	void set_parent_pointers_for_me_and_my_decendents(AST_node_ *my_parent);

	virtual std::list<AST_node_*> getChildren();

	virtual void send_or_recieve_ST(ST_bi new_ST);	  // passes ST to it's parent
	virtual void send_or_recieve_SP(int new_SP) {			// gets new SP and passes to parent
		this->set_SP(new_SP);
		this->parent()->send_or_recieve_SP(new_SP);
	}
...
public:
	...
	virtual void send_or_recieve_ST(ST_bi new_ST); 	// passes ST to it's parent
	virtual void send_or_recieve_SP(int new_SP) {		// passes SP to it's parent
		this->parent()->send_or_recieve_SP(new_SP);
	}
